...
const DashboardPage = () => {
  ...
  const totalCouponsAvaliable = FN_UTILS_NUMBER.sum_all_coupons_avaliable(
    couponsQuery.data?.data?.results ?? []
  )
  return (
        ...
        <div className="grid base:grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <NonStaffOnly>
            <BaseStats
              ...
              Icon={CreditCard}
              value={
                totalCouponsAvaliable -
                (redemptionsQuery.data?.data?.count ?? 0)
              }
              loading={couponsQuery.isLoading}
          ...
          <TabsContent value="overview" className="space-y-3">
            <StaggeredFade className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              <BaseCard
                ...
                Icon={Activity}
                description="Últimos resgates realizados"
                footer={
                  <BaseButton variant="outline" href="/my-coupons">
                    Ver todos
                  </BaseButton>
                }
              >
                ...
                >
                  {recentRedemptionsQuery.data?.data?.length ? (
                    <StaggeredFade className="space-y-3">
                      {recentRedemptionsQuery.data.data
                        .slice(0, 5)
                        .map((redemption: any) => (
                          <div
                            key={redemption.id}
              ...
              >
                <div className="space-y-3">
                  <BaseButton variant="outline" Icon={Gift} href="/coupons">
                    Gerenciar Cupons
                  </BaseButton>
                  ...
                  <StaffOnly>
                    ...
                    <BaseButton variant="outline" Icon={Users} href="/users">
                      Usuários
                    </BaseButton>
                    <BaseButton variant="outline" Icon={Settings} href="/admin">
                      Configurações
                    </BaseButton>
                  </StaffOnly>
          ...
          <TabsContent value="coupons" className="space-y-3">
            <StaggeredFade>
              <BaseCard
                title="Cupons Disponíveis"
                Icon={Gift}
                description="Lista dos cupons mais recentes"
                footer={
                  <BaseButton variant="outline" href="/coupons">
                    Ver todos
                  </BaseButton>
                }
              >
                      ...
                      {couponsQuery.data.data.results
                        ...
                        .map((coupon: any) => (
                          ...
                          >
                            ...
                            <Badge
                              variant={
                                coupon.available ? 'default' : 'secondary'
                              }
                            >
          ...
          <TabsContent value="redemptions" className="space-y-3">
            <StaggeredFade>
              <BaseCard
                ...
                Icon={Activity}
                description="Todos os resgates realizados"
                footer={
                  <BaseButton variant="outline" href="/my-coupons">
                    Ver todos
                  </BaseButton>
                ...
                <div className="space-y-3">
                  ...
                  >
                    {redemptionsQuery.data?.data?.results?.length ? (
                      <StaggeredFade className="space-y-3">
                        {redemptionsQuery.data.data.results
                          .slice(0, 5)
                          .map((redemption: any) => (
                            <div
                              key={redemption.id}
          ...
          <StaffOnly>
            <TabsContent value="admin" className="space-y-3">
              <BaseCard
                ...
                Icon={Shield}
                description="Ferramentas de administração do sistema"
              >
                <div className="space-y-3">
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg">
                    ...
                    <div className="text-sm space-y-1">
                      ...
                      <p>
                        <strong>Cupons Criados:</strong>{' '}
                        {couponsQuery.data?.data?.count || 0}
                      </p>
                      <p>
                        <strong>Resgates Totais:</strong>{' '}
                        {redemptionsQuery.data?.data?.count || 0}
                      </p>
                    </div>
                  </div>
